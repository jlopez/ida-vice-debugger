...
//--------------------------------------------------------------------------
const char *get_mon_response_name(BinaryMonitorCommandResponse code) {
  if (code == MON_RESPONSE_INVALID)
    return "MON_RESPONSE_INVALID";
  if (code == MON_RESPONSE_MEM_GET)
    return "MON_RESPONSE_MEM_GET";
  if (code == MON_RESPONSE_MEM_SET)
    return "MON_RESPONSE_MEM_SET";
  if (code == MON_RESPONSE_CHECKPOINT_INFO)
    return "MON_RESPONSE_CHECKPOINT_INFO";
  if (code == MON_RESPONSE_CHECKPOINT_DELETE)
    return "MON_RESPONSE_CHECKPOINT_DELETE";
  if (code == MON_RESPONSE_CHECKPOINT_LIST)
    return "MON_RESPONSE_CHECKPOINT_LIST";
  if (code == MON_RESPONSE_CHECKPOINT_TOGGLE)
    return "MON_RESPONSE_CHECKPOINT_TOGGLE";
  if (code == MON_RESPONSE_CONDITION_SET)
    return "MON_RESPONSE_CONDITION_SET";
  if (code == MON_RESPONSE_REGISTER_INFO)
    return "MON_RESPONSE_REGISTER_INFO";
  if (code == MON_RESPONSE_DUMP)
    return "MON_RESPONSE_DUMP";
  if (code == MON_RESPONSE_UNDUMP)
    return "MON_RESPONSE_UNDUMP";
  if (code == MON_RESPONSE_RESOURCE_GET)
    return "MON_RESPONSE_RESOURCE_GET";
  if (code == MON_RESPONSE_RESOURCE_SET)
    return "MON_RESPONSE_RESOURCE_SET";
  if (code == MON_RESPONSE_JAM)
    return "MON_RESPONSE_JAM";
  if (code == MON_RESPONSE_STOPPED)
    return "MON_RESPONSE_STOPPED";
  if (code == MON_RESPONSE_RESUMED)
    return "MON_RESPONSE_RESUMED";
  if (code == MON_RESPONSE_ADVANCE_INSTRUCTIONS)
    return "MON_RESPONSE_ADVANCE_INSTRUCTIONS";
  if (code == MON_RESPONSE_KEYBOARD_FEED)
    return "MON_RESPONSE_KEYBOARD_FEED";
  if (code == MON_RESPONSE_EXECUTE_UNTIL_RETURN)
    return "MON_RESPONSE_EXECUTE_UNTIL_RETURN";
  if (code == MON_RESPONSE_PING)
    return "MON_RESPONSE_PING";
  if (code == MON_RESPONSE_BANKS_AVAILABLE)
    return "MON_RESPONSE_BANKS_AVAILABLE";
  if (code == MON_RESPONSE_REGISTERS_AVAILABLE)
    return "MON_RESPONSE_REGISTERS_AVAILABLE";
  if (code == MON_RESPONSE_DISPLAY_GET)
    return "MON_RESPONSE_DISPLAY_GET";
  if (code == MON_RESPONSE_VICE_INFO)
    return "MON_RESPONSE_VICE_INFO";
  if (code == MON_RESPONSE_PALETTE_GET)
    return "MON_RESPONSE_PALETTE_GET";
  if (code == MON_RESPONSE_JOYPORT_SET)
    return "MON_RESPONSE_JOYPORT_SET";
  if (code == MON_RESPONSE_USERPORT_SET)
    return "MON_RESPONSE_USERPORT_SET";
  if (code == MON_RESPONSE_EXIT)
    return "MON_RESPONSE_EXIT";
  if (code == MON_RESPONSE_QUIT)
    return "MON_RESPONSE_QUIT";
  if (code == MON_RESPONSE_RESET)
    return "MON_RESPONSE_RESET";
  if (code == MON_RESPONSE_AUTOSTART)
    return "MON_RESPONSE_AUTOSTART";
  return "???";
}

const char *get_mon_command_name(BinaryMonitorCommand code) {
  if (code == MON_CMD_INVALID)
    return "MON_CMD_INVALID";
  if (code == MON_CMD_MEM_GET)
    return "MON_CMD_MEM_GET";
  if (code == MON_CMD_MEM_SET)
    return "MON_CMD_MEM_SET";
  if (code == MON_CMD_CHECKPOINT_GET)
    return "MON_CMD_CHECKPOINT_GET";
  if (code == MON_CMD_CHECKPOINT_SET)
    return "MON_CMD_CHECKPOINT_SET";
  if (code == MON_CMD_CHECKPOINT_DELETE)
    return "MON_CMD_CHECKPOINT_DELETE";
  if (code == MON_CMD_CHECKPOINT_LIST)
    return "MON_CMD_CHECKPOINT_LIST";
  if (code == MON_CMD_CHECKPOINT_TOGGLE)
    return "MON_CMD_CHECKPOINT_TOGGLE";
  if (code == MON_CMD_CONDITION_SET)
    return "MON_CMD_CONDITION_SET";
  if (code == MON_CMD_REGISTERS_GET)
    return "MON_CMD_REGISTERS_GET";
  if (code == MON_CMD_REGISTERS_SET)
    return "MON_CMD_REGISTERS_SET";
  if (code == MON_CMD_DUMP)
    return "MON_CMD_DUMP";
  if (code == MON_CMD_UNDUMP)
    return "MON_CMD_UNDUMP";
  if (code == MON_CMD_RESOURCE_GET)
    return "MON_CMD_RESOURCE_GET";
  if (code == MON_CMD_RESOURCE_SET)
    return "MON_CMD_RESOURCE_SET";
  if (code == MON_CMD_ADVANCE_INSTRUCTIONS)
    return "MON_CMD_ADVANCE_INSTRUCTIONS";
  if (code == MON_CMD_KEYBOARD_FEED)
    return "MON_CMD_KEYBOARD_FEED";
  if (code == MON_CMD_EXECUTE_UNTIL_RETURN)
    return "MON_CMD_EXECUTE_UNTIL_RETURN";
  if (code == MON_CMD_PING)
    return "MON_CMD_PING";
  if (code == MON_CMD_BANKS_AVAILABLE)
    return "MON_CMD_BANKS_AVAILABLE";
  if (code == MON_CMD_REGISTERS_AVAILABLE)
    return "MON_CMD_REGISTERS_AVAILABLE";
  if (code == MON_CMD_DISPLAY_GET)
    return "MON_CMD_DISPLAY_GET";
  if (code == MON_CMD_VICE_INFO)
    return "MON_CMD_VICE_INFO";
  if (code == MON_CMD_PALETTE_GET)
    return "MON_CMD_PALETTE_GET";
  if (code == MON_CMD_JOYPORT_SET)
    return "MON_CMD_JOYPORT_SET";
  if (code == MON_CMD_USERPORT_SET)
    return "MON_CMD_USERPORT_SET";
  if (code == MON_CMD_EXIT)
    return "MON_CMD_EXIT";
  if (code == MON_CMD_QUIT)
    return "MON_CMD_QUIT";
  if (code == MON_CMD_RESET)
    return "MON_CMD_RESET";
  if (code == MON_CMD_AUTOSTART)
    return "MON_CMD_AUTOSTART";
  return "???";
}
