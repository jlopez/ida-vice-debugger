...
//--------------------------------------------------------------------------
const char *get_event_name_old(event_id_t id)
{
  switch ( id )
  {
...
char *debug_event_str(const debug_event_t *ev, char *buf, size_t bufsize)
{
  ...
  ptr += qsnprintf(ptr, end-ptr, "%s ea=%llu",
                   get_event_name_old(ev->eid),
                   ev->ea);
  switch ( ev->eid )
  {
  ...
  case LIBRARY_LOAD:   // New library loaded
    ptr += qsnprintf(ptr, end-ptr, " base=%llu size=%llu rebase=%llu name=%s",
                     ev->modinfo.base,
                     ev->modinfo.size,
  ...
  case BREAKPOINT:     // Breakpoint reached
    ptr += qsnprintf(ptr, end-ptr, " hea=%llu kea=%llu", ev->bpt.hea, ev->bpt.kea);
    break;
  case EXCEPTION:      // Exception
    ptr += qsnprintf(ptr, end-ptr, " code=%x can_cont=%d ea=%llu info=%s",
                     ev->exc.code,
                     ev->exc.can_cont,
                     ev->exc.ea,
                     ev->exc.info);
    ...
    || lc->type != BPT_SOFT && find_page_bpt(lc->ea, lc->size) != page_bpts.end() )
  {
    ...
    if ( code > 0 )
    {
      ...
      if ( code > 0 )
      {
        ...
        if ( code <= 0 )
          dmsg("%llu: failed to single step\n", event->ea); // may happen
        ...
        if ( dbg_add_bpt(lc->type, lc->ea, bptlen) <= 0 )
        {
          ...
          // any more
          code = 0;
          dwarning("%llu: could not restore deleted bpt\n", lc->ea); // odd
        }
      }
...
bool debmod_t::evaluate_and_handle_lowcnd(debug_event_t *event, int elc_flags)
{
  ...
  if ( event->eid == BREAKPOINT )
  {
    ...
    { // condition is not satisfied, just make a single step and resume
      debdeb("%llX: bptcnd yielded false\n", ea);
      event->handled = true;
        ...
        int ndel)
{
  ...
  for ( b=ubpts; b != end; b++ )
  {
    ...
    if ( code == BPT_OK )
    {
      ...
      debdeb("dbg_add_bpt(type=%d, ea=%llX, len=%d) => %d\n", b->type, ea, len, code);
      switch ( code )
      {
        case 2:
          code = BPT_PAGE_OK;
  ...
  for ( ; b != end; b++ )
  {
    ...
    debdeb("dbg_del_bpt(type=%d, ea=%llu) => %d\n", b->type, b->ea, code);
    if ( code > 0 )
    {
      cnt++;
      if ( b->type == BPT_SOFT )
...
bool debmod_t::restore_broken_breakpoints(void)
{
  ...
  for ( p = bpts.begin(); p != bpts.end(); ++p )
  {
    ...
    if ( !dbg_write_memory(bpt.ea, bpt.saved, bpt.nsaved) )
      msg("Failed to restore broken breakpoint at 0x%llX\n", bpt.ea);
  }
  bpts.clear();
  return true;
}
        ...
        const exception_info_t *ei)
{
  if ( ei == NULL || (ei->flags & EXC_SILENT) == 0 )
  {
    ...
    {
      dmsg("%08llX: %s -> %08llX (exc.code %x, tid %d)\n",
           ev->ea, ev->exc.info, ev->exc.ea, ev->exc.code, ev->tid);
    }
    else
    {
      dmsg("%08llX: %s (exc.code %x, tid %d)\n",
