...
//--------------------------------------------------------------------------
const char *m6502_register_classes[] =
{
  "General registers",
  "Synthetic registers",
  NULL
...
{
  M6502_RC_GENERAL          = 0x01,
  M6502_RC_SYNTHETIC        = 0x02,
  M6502_RC_ALL              = M6502_RC_GENERAL | M6502_RC_SYNTHETIC,
};

static const char *const p_flags[] =
{
  "C", "Z", "I", "D", "B", nullptr, "V", "N", nullptr
};

register_info_t m6502_registers[] =
{
  // General registers
  { "A",   0,                              M6502_RC_GENERAL  ,  dt_byte, nullptr, 0 },
  { "X",   0,                              M6502_RC_GENERAL  ,  dt_byte, nullptr, 0 },
  { "Y",   0,                              M6502_RC_GENERAL  ,  dt_byte, nullptr, 0 },
  { "PC",  REGISTER_IP | REGISTER_ADDRESS, M6502_RC_GENERAL  ,  dt_word, nullptr, 0 },
  { "S",   REGISTER_SP,                    M6502_RC_GENERAL  ,  dt_word, nullptr, 0 },
  { "P",   0,                              M6502_RC_GENERAL  ,  dt_byte, p_flags, 0 },

  // Synthetic registers
  { "LIN", 0,                              M6502_RC_SYNTHETIC,  dt_word, nullptr, 0 },
  { "CYC", 0,                              M6502_RC_SYNTHETIC,  dt_word, nullptr, 0 },
  { "00",  0,                              M6502_RC_SYNTHETIC,  dt_byte, nullptr, 0 },
  { "01",  0,                              M6502_RC_SYNTHETIC,  dt_byte, nullptr, 0 },
};
...
int idaapi x86_read_registers(
        ...
        regval_t *values)
{
  msg("*** %s(thread_id=%d, clsmask=0x%02X)\n", __func__, thread_id, clsmask);
...
{
  msg("*** %s(thread_id=%d, reg_idx=%d %s)\n", __func__, thread_id, reg_idx, x86_registers[reg_idx].name);
  regval_t rv = *value;
...
int is_x86_valid_bpt(bpttype_t type, ea_t ea, int len)
{
  if ( type != BPT_SOFT )
  {
    if ( (dbg->flags & DBG_FLAG_ANYSIZE_HWBPT) == 0 )
